...
export default function LqEqCheckerPage() {
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({"@context":"https://schema.org","@type":"WebApplication","name":"ADR LQ/EQ Checker","description":"Check if dangerous goods qualify for ADR Limited Quantity (LQ) or Excepted Quantity (EQ) exemptions. Batch multi-item eligibility check. Free, ADR 2025.","url":"https://www.freightutils.com/adr/lq-eq-checker","applicationCategory":"UtilityApplication","operatingSystem":"All","offers":{"@type":"Offer","price":"0","priceCurrency":"GBP"},"author":{"@type":"Person","name":"Marius Cristoiu","url":"https://www.linkedin.com/in/marius-cristoiu-a853812a2/"}}) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"What are Limited Quantities in ADR?","acceptedAnswer":{"@type":"Answer","text":"ADR Chapter 3.4 allows dangerous goods to be transported with reduced requirements when packaged in small quantities. Each UN number has a maximum quantity per inner packaging (Column 7a of Table A). Qualifying goods get simplified labelling, no Tremcard, and reduced driver training requirements."}},{"@type":"Question","name":"What are Excepted Quantities in ADR?","acceptedAnswer":{"@type":"Answer","text":"ADR Chapter 3.5 provides even more relaxed requirements for very small quantities. Each substance has an EQ code (E1-E5) defining maximum quantities per inner and outer packaging. E0 means no EQ exemption. EQ transport is exempt from most ADR requirements except basic packaging and labelling."}},{"@type":"Question","name":"What is the difference between LQ and EQ?","acceptedAnswer":{"@type":"Answer","text":"Limited Quantities (ADR 3.4) are for retail-chain distribution with relatively generous limits (1-5 L/kg per inner packaging). Excepted Quantities (ADR 3.5) cover much smaller amounts for laboratory samples and diagnostics (1-30 ml/g inner). EQ has stricter packaging but fewer transport requirements."}}]}) }} />
      <PageHero
        title="LQ/EQ"
        titleAccent="Checker"
      ...
      <main data-category="dg" style={{ maxWidth: 900, margin: '0 auto', padding: '24px 20px 80px' }}>
        <ErrorBoundary>
          <LqEqChecker />
        </ErrorBoundary>
